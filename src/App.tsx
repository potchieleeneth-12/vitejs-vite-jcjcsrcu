  ...
  const handleCoverPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/.netlify/functions/gemini', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: 'Identify the book in this cover image. Return ONLY a JSON object: {"title":"...","author":"..."}. If unknown return {"title":"","author":""}. Do not include markdown.',
          image: { b64, mime }
        }),
      });
     ...
     {randomPick && randomPick.length > 0 && (
        <div style={{ position:'fixed',inset:0,zIndex:70,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.85)',padding:'1rem' }}>
          <div style={{ background:'#0e0b1a',border:'1px solid rgba(251,146,60,0.4)',borderRadius:'1.2rem',padding:'1.5rem',width:'100%',maxWidth:'460px',maxHeight:'85vh',overflowY:'auto' }}>
            ...
            <div style={{ display:'flex',gap:'0.75rem' }}>
              <button onClick={pickRandom} style={{ flex:1,background:'rgba(251,146,60,0.18)',color:'#fb923c',border:'1px solid rgba(251,146,60,0.4)',borderRadius:'0.75rem',padding:'0.65rem',cursor:'pointer',fontWeight:600,fontSize:'0.82rem' }}>Reshuffle 🎲</button>
              <button onClick={() => setRandomPick(null)} style={{ flex:1,background:'rgba(255,255,255,0.05)',color:'rgba(255,255,255,0.5)',border:'none',borderRadius:'0.75rem',padding:'0.65rem',cursor:'pointer',fontSize:'0.82rem' }}>Close</button>
            </div>
          </div>
        </div>
      )}
