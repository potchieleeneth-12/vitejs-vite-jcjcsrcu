...
const saveToFirestore = async (uid, books, goals) => {
...
const GENRE_CFG = {
  ...
  Contemporary: { accent: '#34d399', dim: '#064e3b' },
...
const STATUS_COLORS = { shelf: '#a78bfa', tbr: '#fb923c', reading: '#34d399' };

// ── Helpers ───────────────────────────────────────────────────────────────────
const uid = () => Date.now() + Math.random();
const fa = (id, t, a, sg, sr, sn) => ({
  id,
  title: t,
  author: a,
  category: 'Fiction',
...
function StatCard({ label, value, sub, color = '#a78bfa' }) {
  return (
    ...
    >
      <div
        style={{ color, fontSize: '1.6rem', fontWeight: 'bold', lineHeight: 1 }}
      >
        {value}
      </div>
      <div
      ...
      {sub && (
        <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.6rem' }}>
          {sub}
        </div>
      )}
    </div>
...
// ── GoalSetModal ──────────────────────────────────────────────────────────────
function GoalSetModal({ goals, onSave, onClose }) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button
            onClick={() =>
              onSave({ yearly: Number(y) || 0, monthly: Number(mo) || 0 })
            }
            style={{
              flex: 1,
              background: '#6d28d9',
...
// ── HomeTab ───────────────────────────────────────────────────────────────────
function HomeTab({ books, goals, onEditGoals }) {
  const now = new Date();
  ...
  const thisMonthRead = readAll.filter((b) => {
  ...
  const yearlyPct = goals.yearly
    ? Math.min(100, Math.round((readAll.length / goals.yearly) * 100))
    : 0;
  const monthlyPct = goals.monthly
    ? Math.min(100, Math.round((thisMonthRead.length / goals.monthly) * 100))
    : 0;
  ...
  const genreData = useMemo(() => {
    ...
    return Object.entries(c)
      .map(([g, n]) => ({
        genre: g,
        count: n,
        color: GENRE_CFG[g]?.accent || '#a78bfa',
      }))
  ...
  const authorData = useMemo(() => {
    const c = {};
    readAll.forEach((b) => {
      c[b.author] = (c[b.author] || 0) + 1;
    });
    return Object.entries(c)
      .map(([a, n]) => ({ author: a, count: n }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 8);
  }, [readAll]);
  ...
  const readPct = books.length
    ? Math.round((readAll.length / books.length) * 100)
    : 0;
  ...
  return (
    <div style={{ padding: '1rem', maxWidth: '960px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <div
          style={{ color: '#e8d9ff', fontSize: '1.25rem', fontWeight: 'bold' }}
        >
          ✦ My Reading Life
        </div>
      ...
      <div style={card}>
        ...
        >
          ...
          >
            {readAll.length} of {books.length} books
          </span>
        </div>
        <div
          style={{
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: '0.75rem' }}>
          ...
          >
            ...
            >
              {goals.yearly
                ? `${readAll.length} / ${goals.yearly} books`
                : 'No goal set'}
            </span>
          </div>
          <div
        ...
        <div>
          ...
          >
            ...
            >
              {goals.monthly
                ? `${thisMonthRead.length} / ${goals.monthly} books`
                : 'No goal set'}
            </span>
          </div>
          <div
          ...
          {goals.monthly > 0 && (
            ...
            >
              {monthlyPct}% complete ·{' '}
              {Math.max(0, goals.monthly - thisMonthRead.length)} left this
              month
            </div>
          )}
        </div>
        ...
        <div style={card}>
          ...
          {genreData.map(({ genre, count, color }) => (
            <div key={genre} style={{ marginBottom: '0.5rem' }}>
              ...
              >
                <span style={{ fontSize: '0.72rem', color: color }}>
                  {genre}
                </span>
                <span
                  style={{
      ...
      {genreData.length === 0 && (
        ...
        >
          ...
          <p style={{ fontSize: '0.85rem' }}>
            Mark books as read to see your stats here.
          </p>
        </div>
      )}
    </div>
...
// ── ModalForm ─────────────────────────────────────────────────────────────────
function ModalForm({ book, onSave, onSaveMany, onClose, tab, allSeries }) {
  const [mode, setMode] = useState('single'); // "single" | "bulk" | "photo"

  // ── shared shelf/bulk settings ──────────────────────────────────────────────
  ...
  const blank = {
    ...
    sn: '',
    read: false,
    status: tab === 'home' ? 'shelf' : tab,
    readAt: null,
  };
  const [f, setF] = useState(
    book ? { ...book, sn: book.sn ?? '', series: book.series ?? '' } : blank
  );
          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: 'Identify the book. Return ONLY JSON: {"title":"...","author":"..."}. Unknown: {"title":"","author":""}.',
                },
              ],
            },
          ],
  ...
  const submitBulk = () => {
    const valid = bulkParsed.filter((p) => p.title.trim());
    ...
    onSaveMany(
      valid.map((p) => ({
        ...
        category: 'Fiction',
        genre: shelfGenre,
        subgenre: SUBGENRES[shelfGenre]?.[0] || '',
        series: null,
        sn: null,
        read: shelfRead,
        readAt: shelfRead ? Date.now() : null,
        status: shelfStatus,
      }))
    );
    setBulkDone(true);
  ...
  const [shelfImg, setShelfImg] = useState(null);
  const [shelfB64, setShelfB64] = useState('');
  const [shelfMime, setShelfMime] = useState('');
  const [scanning, setScanning] = useState(false);
  const [scanErr, setScanErr] = useState('');
  const [scanned, setScanned] = useState([]); // [{title,author,selected}]
  const [scanDone, setScanDone] = useState(false);

  const handleShelfFile = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const b64 = await fileToBase64(file);
    setShelfB64(b64);
    setShelfMime(file.type);
              ...
              content: [
                {
                  ...
                  source: {
                    type: 'base64',
                    media_type: shelfMime,
                    data: shelfB64,
                  },
                },
                {
                  type: 'text',
                  text: `Look at every single book spine visible in this bookshelf photo. Read each title and author printed on the spines carefully.
Return ONLY a raw JSON array with no markdown, no backticks, no explanation:
[{"title":"Exact Title","author":"Author Name"},...]
Rules:
- Include EVERY book spine you can read, even partially
- If you can read the title but not the author, use empty string for author
- Preserve exact capitalization from the spine
- Do not skip any books`,
      ...
      setScanErr(
        "Couldn't read the shelf \u2014 try a clearer photo with good lighting."
      );
    }
    setScanning(false);
  };
  ...
  const toggleOne = (i) =>
    setScanned((p) =>
      p.map((b, j) => (j === i ? { ...b, selected: !b.selected } : b))
    );
  const toggleAll = (v) =>
    setScanned((p) => p.map((b) => ({ ...b, selected: v })));
  const editOne = (i, k, val) =>
    setScanned((p) => p.map((b, j) => (j === i ? { ...b, [k]: val } : b)));
  const selectedCount = scanned.filter((b) => b.selected).length;

  const submitScan = () => {
    const chosen = scanned.filter((b) => b.selected && b.title.trim());
    ...
    <>
      ...
      >
        <div>
          ...
          <select
            value={shelfGenre}
            onChange={(e) => setShelfGenre(e.target.value)}
            style={{ ...inp, background: '#1a1035' }}
          >
      ...
      {shelfStatus === 'shelf' && (
        ...
        >
          <input
            type="checkbox"
            checked={shelfRead}
            onChange={(e) => setShelfRead(e.target.checked)}
          ...
          <>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    set('subgenre', SUBGENRES[e.target.value]?.[0] || '');
                  }}
                  style={{ ...inp, background: '#1a1035' }}
                >
                  {Object.keys(SUBGENRES).map((g) => (
                  ...
                  .map(([k, cfg]) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '0.6rem',
                        border: `1px solid ${
                          f.status === k ? cfg.color : 'rgba(255,255,255,0.1)'
                        }`,
                        background:
                          f.status === k ? cfg.color + '25' : 'transparent',
                        color:
                          f.status === k ? cfg.color : 'rgba(255,255,255,0.4)',
                        cursor: 'pointer',
                        fontSize: '0.68rem',
                        fontWeight: f.status === k ? 700 : 400,
                      }}
                    >
                      {k === 'shelf'
                        ? '\ud83d\udcda Shelf'
            ...
            {f.status === 'shelf' && (
              ...
              >
                <input
                  type="checkbox"
                  checked={f.read || false}
                  onChange={(e) => set('read', e.target.checked)}
                  style={{ accentColor: '#7c3aed' }}
                />
                <span
                  style={{
        ...
        {mode === 'bulk' && (
          <>
            <div style={{ marginBottom: '0.6rem' }}>
              ...
              >
                Paste your list \u2014 one book per line
              </label>
              <div
                style={{
                  fontSize: '0.65rem',
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                onClick={submitBulk}
                disabled={bulkParsed.length === 0 || bulkDone}
                style={{
                  flex: 1,
                  background: bulkDone ? '#059669' : '#6d28d9',
                  color: 'white',
                  border: 'none',
                  borderRadius: '0.75rem',
                  padding: '0.6rem',
              ...
              >
                {bulkDone
                  ? `\u2713 Added ${bulkParsed.length} books!`
                  : `Add ${bulkParsed.length || 0} book${
                      bulkParsed.length !== 1 ? 's' : ''
                    }`}
              </button>
              <button
                onClick={onClose}
          ...
          <>
            ...
            {!shelfImg ? (
              ...
              >
                ...
                >
                  Works best with clear, well-lit spines
                </span>
              </button>
            ) : (
            ...
            {scanning && (
              ...
              >
                ...
                >
                  Scanning your shelf\u2026 this may take a moment
                </div>
              </div>
            )}
            ...
            {scanned.length > 0 && (
              <>
                  ...
                  {scanned.map((b, i) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <input
                          value={b.title}
                          onChange={(e) => editOne(i, 'title', e.target.value)}
                          style={{
                            ...inp,
                        ...
                        <input
                          value={b.author}
                          onChange={(e) => editOne(i, 'author', e.target.value)}
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button
                    onClick={submitScan}
                    disabled={selectedCount === 0 || scanDone}
                    style={{
                      flex: 1,
                      background: scanDone ? '#059669' : '#6d28d9',
                      color: 'white',
                      border: 'none',
                      borderRadius: '0.75rem',
                      padding: '0.65rem',
                  ...
                  >
                    {scanDone
                      ? `\u2713 Added ${selectedCount} books!`
                      : `Add ${selectedCount} book${
                          selectedCount !== 1 ? 's' : ''
                        }`}
                  </button>
                  <button
...
export default function App() {
  ...
  const [goals, setGoals] = useState({ yearly: 0, monthly: 0 });
  ...
  // ── load — Firebase if configured, otherwise localStorage ──
  useEffect(() => {
    (async () => {
      const fbOk = await initFirebase();

      if (fbOk) {
        // Firebase available — set up auth listener
        const unsub = _onAuthStateChanged(_auth, async (u) => {
          setUser(u);
          setAuthReady(true);
          if (u) {
            setSyncing(true);
            try {
              ...
              if (snap.exists()) {
                ...
                const cloudBooks = (data.books || []).map((b) => ({
                  ...b,
                  status: b.status || 'shelf',
                  readAt: b.readAt || null,
                }));
                const ids = new Set(cloudBooks.map((b) => b.id));
                setBooks([
                  ...cloudBooks,
                  ...ALL_BOOKS.filter((b) => !ids.has(b.id)),
                ]);
                ...
                setBooks(merged);
                let lg = { yearly: 0, monthly: 0 };
                try {
                  const g = localStorage.getItem(GOALS_KEY);
                  if (g) lg = JSON.parse(g);
                } catch {}
                setGoals(lg);
            ...
            try {
              ...
              if (raw) {
                const s = JSON.parse(raw).map((b) => ({
                  ...b,
                  status: b.status || 'shelf',
                  readAt: b.readAt || null,
                }));
                const ids = new Set(s.map((b) => b.id));
                setBooks([...s, ...ALL_BOOKS.filter((b) => !ids.has(b.id))]);
              } else {
                setBooks(ALL_BOOKS);
              }
            } catch {
        ...
        // No Firebase — use localStorage only, skip sign-in screen
        try {
          const keys = [
            STORAGE_KEY,
            'myshelf-v5',
  ...
  const handleSignIn = () => {
    if (firebaseReady) _signInWithPopup(_auth, _provider).catch(() => {});
  };
  const handleSignOut = () => {
    if (firebaseReady) _signOut(_auth);
  };

  const update = (id, patch) => {
    const extra = {};
    if (patch.read === true) extra.readAt = Date.now();
    if (patch.read === false) extra.readAt = null;
    persist(books.map((b) => (b.id === id ? { ...b, ...patch, ...extra } : b)));
  };

  // ── filtered books per tab ──
  const tabBooks = useMemo(() => {
    if (tab === 'home') return [];
    if (tab === 'shelf') return books; // shelf shows ALL books
    if (tab === 'tbr') return books.filter((b) => b.status === 'tbr');
    if (tab === 'reading') return books.filter((b) => b.status === 'reading');
    return books;
  }, [books, tab]);

  const allGenres = useMemo(
    () => [...new Set(tabBooks.map((b) => b.genre))].sort(),
    [tabBooks]
  );
  const allSubs = useMemo(() => {
    const src =
      fGenre === 'All' ? tabBooks : tabBooks.filter((b) => b.genre === fGenre);
    return [...new Set(src.map((b) => b.subgenre).filter(Boolean))].sort();
  }, [tabBooks, fGenre]);
  const allSeries = useMemo(
    () => [...new Set(books.map((b) => b.series).filter(Boolean))].sort(),
    [books]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return tabBooks
      .filter((b) => {
        ...
        if (tab === 'shelf' && fRead !== 'All' && (fRead === 'Read') !== b.read)
          return false;
        if (fSeries !== 'All' && b.series !== fSeries) return false;
        return true;
      })
      .sort((a, b) => a.title.localeCompare(b.title)); // always alphabetical
  ...
  const hasFilter =
    fGenre !== 'All' || fSub !== 'All' || fRead !== 'All' || fSeries !== 'All';
  const clearFilters = () => {
    setFGenre('All');
    setFSub('All');
    setFRead('All');
  ...
  // ── Sign-in screen (only when Firebase is configured) ──
  ...
  return (
    ...
    >
      {/* ── STICKY HEADER (includes filters) ── */}
      <div
        style={{
          background: '#0d0a1c',
        ...
        >
          ...
          >
            <div>
              ...
              >
                {counts.shelf} books · {counts.read} read · {counts.tbr} TBR ·{' '}
          ...
          {/* tabs */}
          <div
            style={{ display: 'flex', gap: '0.3rem', marginBottom: '0.4rem' }}
          >
            {Object.entries(TAB_CFG).map(([k, cfg]) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: '0.65rem',
                  border: `1px solid ${
                    tab === k ? cfg.color : 'rgba(255,255,255,0.08)'
                  }`,
                  background: tab === k ? cfg.color + '22' : 'transparent',
                  color: tab === k ? cfg.color : 'rgba(255,255,255,0.35)',
                  cursor: 'pointer',
                  fontSize: '0.65rem',
          ...
          {/* search + filter controls — only for non-home tabs */}
          {tab !== 'home' && (
            <>
              <input
                value={search}
                    ...
                    >
                      {['All', ...allGenres].map((o) => (
                        <Pill
                          ...
                          label={o}
                          active={fGenre === o}
                          color={GENRE_CFG[o]?.accent || tabColor}
                          onClick={() => {
                            setFGenre(o);
                            setFSub('All');
                          }}
                      ...
                      >
                        {['All', ...allSubs].map((o) => (
                          <Pill
                            ...
                            label={o}
                            active={fSub === o}
                            color={GENRE_CFG[fGenre]?.accent || tabColor}
                            onClick={() => setFSub(o)}
                          />
                        ))}
                      </div>
          ...
          {filtered.length === 0 ? (
            ...
            >
              ...
              {(tab === 'tbr' || tab === 'reading') && (
                ...
                >
                  Use the arrow buttons on any card to move books here.
                </p>
              )}
            </div>
          ) : (
            ...
            >
              {filtered.map((b) => {
                ...
                const moves = Object.entries(TAB_CFG).filter(
                  ([k]) => k !== 'home' && k !== bst
                );
                ...
                return (
                  ...
                  >
                    {/* status badge (on shelf tab, shows non-shelf status) */}
                    {tab === 'shelf' && bst !== 'shelf' && (
                      <div
                        style={{
                          position: 'absolute',
                          ...
                          borderRadius: '9999px',
                          background: STATUS_COLORS[bst] + '22',
                          border: `1px solid ${STATUS_COLORS[bst]}`,
                          color: STATUS_COLORS[bst],
                          fontWeight: 600,
                        }}
                      >
                        {bst === 'tbr' ? '🔖 TBR' : '📖 Reading'}
                    ...
                    {/* move buttons (only show when NOT on shelf tab, or on shelf tab for non-shelf items) */}
                    {tab !== 'shelf' && (
                      <div
                        style={{
                          position: 'absolute',
                      ...
                      >
                        {moves.map(([k, c2]) => (
                          <button
                            key={k}
                            onClick={() => update(b.id, { status: k })}
                            style={{
                              fontSize: '0.58rem',
                              padding: '0.15rem 0.4rem',
                              borderRadius: '9999px',
                              border: `1px solid ${c2.color}`,
                    ...
                    {bst === 'shelf' && (
                      <button
                        onClick={() => update(b.id, { read: !b.read })}
                        style={{
                          position: 'absolute',
                          top: '0.5rem',
                          right: '0.5rem',
                    ...
                    {bst === 'reading' && (
                      <button
                        onClick={() =>
                          update(b.id, { status: 'shelf', read: true })
                        }
                        style={{
                          position: 'absolute',
                          top: '0.5rem',
      ...
      {editBook && (
        <ModalForm
          ...
          allSeries={allSeries}
          onSave={(b) => {
            persist(books.map((x) => (x.id === b.id ? b : x)));
            setEditBook(null);
          }}
          onSaveMany={() => {}}
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                onClick={() => {
                  persist(books.filter((b) => b.id !== delId));
                  setDelId(null);
                }}
                style={{
                  flex: 1,
